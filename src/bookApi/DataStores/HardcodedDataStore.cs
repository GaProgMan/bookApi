...
{
    public class HardcodedDataStore : IDataStore
    {
        private readonly List<Book> _fakeDatabase;

        public HardcodedDataStore()
        {
        ...
        public IQueryable<Book> GetAll() => _fakeDatabase.AsQueryable();

        public Book GetById(Guid targetId) => _fakeDatabase.FirstOrDefault(book => book.Id == targetId);
    }
}